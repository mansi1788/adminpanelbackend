...
export const getAllcms = async (req: Request, res: Response) => {
  console.log("Inside getAllcms controller");

  try {
    const { key, title, meta_keyword, body, isActive, sortBy, sortOrder } = req.query;
    ...
    const user = (req as any).user; // middleware user
    const firstname = user?.firstname || "Unknown";
    const lastname = user?.lastname || "User";
    ...
    console.error("Error in getAllcms:", e);
    res.status(500).json({ message: "Server error", e });
  }
};
